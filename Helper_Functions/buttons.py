...
async def get_genre_buttons(view : discord.ui.View, completion_time : int, price_limit : int, tier_number : str, event_name : str, 
    ...
    async def callback(interaction : discord.Interaction, genre_name):
        ...
        ce_id = await get_ce_id(user_id)



        i=0
        while i < num_of_games :
            games.append(get_rollable_game(completion_time, price_limit, tier_number, specific_genre =genre_name, database_tier=database_tier, database_name=database_name, games=games, steamhunters=steamhunters, user_info=database_user[ce_id]))
            i+=1
